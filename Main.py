...
class directory:
    '''directory path'''
    fn_directory = os.path.dirname(os.path.realpath(__file__))


class paths:
    '''Paths to data'''
    fn_characters = directory.fn_directory + '/save/characters.txt'
    fn_accuracy = directory.fn_directory + '/save/accuracy.txt'
    fn_train = directory.fn_directory + '/data/'
    fn_predict = directory.fn_directory + '/data/test.png'
    fn_texts = directory.fn_directory + '/data/texts.txt'
...
def train(network, loader):
    ...
    while True:
        ...
        if character_error < best_error:
            ...
            network.save()
            open(
                paths.fn_accuracy, 'w').write(
                'Network\'s current error rate: %f%%' %
                (character_error * 100))
        else:
            last_improve += 1
...
def test(network, loader):
    ...
    print('Character error rate: %f%% | Word accuracy: %f%%' %
          (error_rate * 100.0, accuracy * 100.0))
    return error_rate
...
def main():
    ...
    parser.add_argument(
        '--train',
        help='Train the Neural Network on the given dataset. For more information, read the README file.',
        action='store_true')
    parser.add_argument(
        '--test',
        help='Test the Neural Network on the given dataset. For more information, read the README file.',
        action='store_true')
    parser.add_argument(
        '--use',
        help='Uses and saves the outputs of the Neural Networks onto CSV files. For more information, read the README file.',
        action='store_true')

    args = parser.parse_args()
    # tells program to train the network
    if args.train:
        loader = Load_Data(
            paths.fn_train,
            Neural_Network.batch_size,
            Neural_Network.image_size,
            Neural_Network.text_len)
        open(paths.fn_characters, 'w').write(str().join(loader.characters))
        open(
            paths.fn_texts,
            'w').write(
            str(' ').join(
                loader.training_words +
                loader.testing_words))

        network = Neural_Network(loader.characters)
        train(network, loader)
    # tells program to test the network
    elif args.test:
        loader = Load_Data(
            paths.fn_train,
            Neural_Network.batch_size,
        ...
        test(network, loader)
    # tells program to use the network
    else:
        print(open(paths.fn_accuracy).read())
        network = Neural_Network(
            open(
                paths.fn_characters).read(),
            saved=args.use,
            restore=True)
