...
class filepath:
    fn_filepath = os.path.dirname(os.path.realpath(__file__))
...
class Neural_Network:
    ...
    def __init__(self, characters, saved=False, restore=False):
        ...
        self.batches = tf.placeholder(
            tf.float32,
            shape=(
                None,
                Neural_Network.image_size[0],
                Neural_Network.image_size[1]))
    ...
    def setup(self):
        ...
        save_dir = filepath.fn_filepath + '/save/'
        ...
        if latest:
            print(
                'Network',
                latest,
                'found. Proceeding to initiate with stored values.')
    ...
    def Convolutional_Neural_Network(self):
        ...
        for layer in range(number_layers):
            weight = tf.Variable(tf.random_normal(
                [weight_values[layer], weight_values[layer], feature_values[layer], feature_values[layer + 1]]))
            conv = tf.nn.relu(
            ...
            data = tf.nn.max_pool(
                ...
                (1,
                 ...
                 1),
                (1,
                 stride_values[layer][0],
                 stride_values[layer][1],
                 1),
                padding='VALID')
    ...
    def Recurrent_Neural_Network(self):
        ...
        ((forward, backward), _) = tf.nn.bidirectional_dynamic_rnn(
            cell_fw=stack, cell_bw=stack, inputs=rec3d_in, dtype=rec3d_in.dtype)
        data = tf.expand_dims(tf.concat([forward, backward], 2), 2)

        # send output to characters
        weight = tf.Variable(tf.random_normal(
            [1, 1, rnn_size * 2, len(self.characters) + 1]))
        self.rec3d_out = tf.squeeze(tf.nn.atrous_conv2d(
            data, weight, 1, padding='SAME'), axis=[2])
    ...
    def Connectionist_Temporal_Classification(self):
        ...
        self.texts = tf.SparseTensor(
            tf.placeholder(
                tf.int64, shape=[
                    None, 2]), tf.placeholder(
                tf.int32, [None]), tf.placeholder(
                    tf.int64, [2]))
        ...
        self.cost = tf.reduce_mean(
            tf.nn.ctc_loss(
                inputs=self.ctc3d_in,
                labels=self.texts,
                sequence_length=self.seq_len,
                ctc_merge_repeated=True))
        self.saved_input = tf.placeholder(
            tf.float32, shape=[
                Neural_Network.text_len, None, len(
                    self.characters) + 1])
        self.character_cost = tf.nn.ctc_loss(
            inputs=self.saved_input,
            labels=self.texts,
            sequence_length=self.seq_len,
            ctc_merge_repeated=True)

        # determine word
        self.wordsearch = tf.nn.ctc_greedy_decoder(
            inputs=self.ctc3d_in, sequence_length=self.seq_len)

    def word_produce(self, ctc_out, batch_size):
        '''Extract texts from CTC'''
        str_codes = [[] for i in range(batch_size)]

        # retrieve Sparse Tensor
        output = ctc_out[0][0]
    ...
    def batching(self, batch):
        ...
        (_,
         cost) = self.sess.run(evals,
                               feed_dict={self.batches: batch.images,
                                          self.texts: sparse,
                                          self.seq_len: [Neural_Network.text_len] * elements,
                                          self.rate: rate})
        self.progress += 1
        return cost
    ...
    def output(self, rnn_out):
        ...
        output_dir = filepath.fn_filepath + '/outputs/'
    ...
    def test_batch(self, batch):
        ...
        evals = self.sess.run(
            eval_req, {
                self.batches: batch.images, self.seq_len: [
                    Neural_Network.text_len] * elements})
        word = evals[0]
    ...
    def save(self):
        ...
        self.saver.save(
            self.sess,
            filepath.fn_filepath +
            '/save/latest',
            global_step=self.current)
